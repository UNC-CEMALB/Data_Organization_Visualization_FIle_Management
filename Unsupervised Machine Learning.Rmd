...
knitr::include_graphics("Module5_4/Module5_4_Image12.png")
```
...
It's important to note that unsupervised machine learning is used for exploratory purposes and just because it can find associations between data points that doesn't necessarily mean that those associations have merit, are indicative of causal relationships, or have biological implications. 
...
+ Dimensionality reduction: Focuses on taking high-dimensional data and transforming it into a lower-dimensional space that has fewer features while preserving important information inherent to the original dataset
...
```{r, echo=FALSE, fig.width=4, fig.height=5}
knitr::include_graphics("Module5_4/Module5_4_Image2.png")
```
...
We can also do this same thing with hiearchical clustering...ADD WORDS.
```{r, fig.height=8, fig.width=10}
chem_hm <- pheatmap(dat_wclusters[,1:10], main = "Heatmap of Physicochemical Properties with Hierarchical Clustering",
                   cluster_rows = TRUE, cluster_cols = FALSE, # now using hierarchical clustering to group the rows
                    scale = "column",              # scaling the data to make differences across chemicals more apparent
                    cutree_row = 2, # adds a break between the 2 largest clusters
                    angle_col = 45, fontsize_col = 7, fontsize_row = 3, # adjusting size and orientation of labels on axes
                    cellheight = 3, cellwidth = 25, # setting height and width for cells
                    border_color = FALSE # specify no border surrounding the cells
)
...
## Principal Component Analysis (PCA)
Next, we will run through some example analyses applying the common data reduction technique of PCA. We'll start by determining how much of the variance is able to be captured within the first two principal components to answer **Environmental Health Question #3**: How do the data compare when physicochemical properties are reduced using PCA? It's recommended to proceed with additional PCA analyses only if at least 60% of the data is able to captured within the first few principal components or eigenvectors [source](INSERT LINK).


We can calculate the principal components across ALL physicochemical data across all chemicals using the `prcomp()` function. Always make sure your data is centered and scaled prior to running to PCA, since it's sensitive to variance.????
```{r}
...
**Answer**: Principal Component 1 captures ~41% of the variance and Principal Component 2 captures ~24% across all physicochemical property values, across all chemicals. These two components together describe ~65% of data and we can proceed with subsequent analyses.
...
And the resulting loading factors of each property's contribution towards each principal component. Note that by default, really small loadings are replaced with empty spaces.
```{r}
my.pca$rotation
```
...
Let's turn our attention to **Environmental Health Question #5**: 5.	If we did not have information telling us which chemical belonged to which class, could we use PCA and k-means to accurately predict whether a chemical is a PFAS vs statin?
...
1. Using k-means clustering, what are the optimal number of clusters we can group these substances into? *Hint*: These methods were not presented in the module, so check out the test your knowledge solution....
...
4.. Construct a PCA plot of the first two Principal Components.
